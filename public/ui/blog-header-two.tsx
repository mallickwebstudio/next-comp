...
export default function BlogHeaderTwo() {
          ...
          >
            <figure itemProp="image" itemScope itemType="https://schema.org/ImageObject">
              <Image
                ...
                src={featureBlog.blogImageSrc}
                width={160}
                height={160}
                alt={featureBlog.title}
              />
              <meta itemProp="url" content={featureBlog.blogImageSrc} />
            </figure>

            <div className="">
              <header className="mt-3">
                <Badge itemProp="articleSection">{featureBlog.category}</Badge>
                <h3 className="mt-2 text-lg font-semibold hover:underline line-clamp-2" itemProp="headline">
                  <Link href={featureBlog.blogHref}>{featureBlog.title}</Link>
                </h3>
              ...
              <div className="mx-auto mt-3 flex gap-3 items-center" itemProp="author" itemScope itemType="https://schema.org/Person">
                <Image
                  ...
                  src={featureBlog.authorImageSrc}
                  width={48}
                  height={48}
                  alt={featureBlog.author}
                  itemProp="image"
                />
                <div className="text-xs">
                  <div className="font-medium" itemProp="name">{featureBlog.author}</div>
                  <div className="flex gap-2">
                    <time dateTime={new Date(featureBlog.date).toISOString()} itemProp="datePublished">
                      {featureBlog.date}
                    </time>
                    <span>•</span>
                    <span>{featureBlog.readTime}</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="mt-8 grid gap-6 sm:grid-cols-2 md:grid-cols-3">
          {filteredBlogs.map((item, index) => (
            <article
              ...
              key={item.title + index + "Card" + "BlogHeaderTwo"}
              itemScope
              itemType="https://schema.org/BlogPosting"
            >
              <figure itemProp="image" itemScope itemType="https://schema.org/ImageObject">
