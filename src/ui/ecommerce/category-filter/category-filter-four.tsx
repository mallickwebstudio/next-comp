...
export default function CategoryFilterFour() {
        ...
        <div className="relative mt-8 md:mt-12">
          ...
          <Dialog open={dialogOpen} onOpenChange={setdialogOpen}>
            <DialogContent>
              ...
              {/* Dialog Body */}
              <div className="px-4 w-full h-[50dvh] overflow-y-scroll">
                <FilterAccordion
                  openItems={openItems}
          ...
          <div className="w-full">
            ...
            <div className="md:sticky md:top-0 py-4 md:-mt-4 bg-background flex flex-col gap-4">
                ...
                <div className="flex gap-2 flex-wrap">
                  {filters.flatMap((filter, index) => {
                    ...
                    return values.map((val) => (
                      <Badge
                        key={filter.id + val + index + "CategoryFilterFour"}
              ...
              <div className="flex gap-2 justify-between">
                ...
                <Button className="flex items-center gap-2 cursor-pointer" variant="outline" size="sm" onClick={() => setdialogOpen(true)}>
                  <ListFilter />
                  Filter
                </Button>
  ...
  return (
    <Accordion className="w-full" type="multiple" value={openItems} onValueChange={setOpenItems}>
      {filters.map((filter, index) => (
        <AccordionItem key={filter.id + index + "CategoryFilterFour"} value={filter.id}>
          ...
          <AccordionContent>
            {filter.type === "checkbox" && (
              <div className="flex flex-col gap-2">
                {filter.options.map((option, index) => (
                  <div key={filter.id + option + index + "CategoryFilterFour"} className="flex items-center gap-2">
                    <Checkbox
            ...
            {filter.type === "radio" && (
              ...
              >
                {filter.options.map((option, index) => (
                  <div key={filter.id + option + index + "CategoryFilterFour"} className="flex items-center space-x-2">
            ...
            {filter.type === "select" && (
              ...
              >
                ...
                <SelectContent>
                  {filter.options.map((option, index) => (
                    <SelectItem className="cursor-pointer" key={filter.id + option + index + "CategoryFilterFour"} value={option}>
                      {option}
                    </SelectItem>
                  ))}
                </SelectContent>
            ...
            {filter.type === "color" && (
              <div className="flex gap-2 mt-2 flex-wrap">
                {filter.options.map((color, index) => (
                  <button
                    key={filter.id + color + index + "CategoryFilterFour"}
